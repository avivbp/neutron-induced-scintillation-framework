...
class DetectorConstruction : public G4VUserDetectorConstruction
{
  ...
  enum class SensorResponseMode { Flat, BuiltinCurve, BuiltinCurveRaw, CsvCurve, CsvCurveRaw };
  ...
  G4double fReflectorReflectivity = 0.95;
...
public:
  ...
  void SetPMTResponseMode(const G4String& mode) {
      ...
      if (m == "flat") {
      ...
      } else if (m == "builtin_curve_raw" || m == "builtin_raw" || m == "as_is" || m == "builtin_curve_as_is") {
          // Use the built-in wavelength-dependent PMT curve exactly as stored in the code.
          // The configured pde/top_pde/bottom_pde values are ignored in this mode.
          fPMTResponseMode = SensorResponseMode::BuiltinCurveRaw;
      } else if (m == "csv_curve_raw" || m == "csv_raw" || m == "file_raw") {
          // Use the user-provided CSV QE/PDE curve as an absolute curve, without scaling to pde.
          ...
          // Default: use built-in curve shape scaled so that its peak equals the configured PDE.
  ...
  void SetSiPMResponseMode(const G4String& mode) {
      ...
      if (m == "flat") {
          ...
          // Use the built-in wavelength-dependent SiPM curve exactly as stored in the code.
          // The configured pde value is ignored in this mode.
