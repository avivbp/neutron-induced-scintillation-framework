...
namespace {
  ...
  namespace {
        ...
        G4double refl)
    {
      ...
      G4cout << "\n=== Sensor optical response table ===" << G4endl;
G4cout << "Reflectivity = " << std::clamp(refl, 0.0, 1.0) << G4endl;
G4cout << "Index\tEnergy [eV]\tWavelength [nm]\tPhysical PDE\tG4 EFFICIENCY"
       << G4endl;

for (size_t i = 0; i < e.size(); ++i) {
    const G4double energy_eV = e[i] / CLHEP::eV;
    const G4double wavelength_nm =
        (CLHEP::h_Planck * CLHEP::c_light / e[i]) / CLHEP::nm;
...
G4cout << "=====================================\n" << G4endl;
  ...
  G4MaterialPropertiesTable* MakePMTMPT(G4double refl, G4double requestedPeakPDE)
  {
    ...
    const auto maxIt =
    std::max_element(efficiency.begin(), efficiency.end());

G4cout << "[MakePMTMPT]"
       << " requestedPeakPDE = " << requestedPeakPDE
       << ", raw/final efficiency max = "
       << (maxIt != efficiency.end() ? *maxIt : -1.0)
       << ", reflectivity = " << refl
       << G4endl;
   ...
   fDetectorMessenger(nullptr),lvol_fiber(nullptr),pvol_fiber(nullptr)
{
  ...
  // Liquid-argon optical dispersion from the visible TPB-emission band
  // through the 128-nm argon-scintillation band.  The visible values are
  // consistent with measurements near 90 K (n ~ 1.23), while the 128-nm
  // point is anchored to the measured value n = 1.358.  The intermediate
  // UV points are a smooth, conservative interpolation; replace them with
  // a dedicated dispersion model if Cherenkov light is itself an observable.
  //
  // Energies are strictly increasing, as required by Geant4.
  std::vector<G4double> energy = {
      1.50*eV, 1.91*eV, 2.07*eV, 2.25*eV, 2.48*eV,
      2.76*eV, 2.95*eV, 3.10*eV, 3.54*eV, 4.13*eV,
      4.96*eV, 6.20*eV, 7.75*eV, 8.86*eV, 9.69*eV,
      9.90*eV
  };

  std::vector<G4double> rindex = {
      1.225, 1.226, 1.226, 1.227, 1.228,
      1.230, 1.232, 1.233, 1.236, 1.242,
      1.255, 1.280, 1.310, 1.335, 1.358,
      1.370
  };

  // Keep the existing configurable absorption-length model, but define it
  // over the full optical range so visible TPB photons are transported too.
  G4double absLen = fLArAbsLength;
  absorptionLength.assign(energy.size(), absLen);
...
{ 
  ...
  // Use the same realistic LAr dispersion in the non-scintillating outer
  // volume.  This allows visible TPB photons to propagate across an
  // inner/outer LAr boundary without assigning n = 1.4 throughout the
  // visible spectrum.
  ...
  std::vector<G4double> absorptionLength3(energy3.size(), absLen3);
...
void DetectorConstruction::UpdateConfigurableOpticalProperties()
{
  ...
  if (lAr) {
    ...
    if (mpt) {
      // Match the full LAr optical-energy grid used when the material table
      // is first constructed.  Only ABSLENGTH is updated here; RINDEX remains
      // the wavelength-dependent table installed in the constructor.
      std::vector<G4double> energy = {
          1.50*eV, 1.91*eV, 2.07*eV, 2.25*eV, 2.48*eV,
          2.76*eV, 2.95*eV, 3.10*eV, 3.54*eV, 4.13*eV,
          4.96*eV, 6.20*eV, 7.75*eV, 8.86*eV, 9.69*eV,
      ...
      absorptionLength.assign(energy.size(), fLArAbsLength);
  ...
  // ============================================================
// Runtime optical-configuration dump switch
//
// Set to 1 to enable the prints.
// Set to 0 to disable them.
// ============================================================
const G4int printOpticalConfig = 1;

if (printOpticalConfig == 1)
{
    G4cout << "\n\n"
           << "============================================================\n"
           << "              RUNTIME OPTICAL CONFIGURATION\n"
           << "============================================================\n";

    // Helper for printing an entire wavelength-dependent property.
    auto printPropertyVector =
        [](const G4String& propertyName,
           const G4MaterialPropertyVector* property,
           const G4String& valueUnitLabel = "",
           G4double valueUnit = 1.0)
    {
        G4cout << "\n[" << propertyName << "]" << G4endl;
        ...
        if (!valueUnitLabel.empty()) {
            G4cout << " [" << valueUnitLabel << "]";
        }

        G4cout << G4endl;

        for (std::size_t i = 0;
             i < property->GetVectorLength();
             ++i) {

            const G4double energy = property->Energy(i);

            const G4double wavelength =
                CLHEP::h_Planck * CLHEP::c_light / energy;

            G4cout << "  " << i
                   << "\t" << energy / CLHEP::eV
                   << "\t" << wavelength / CLHEP::nm
                   << "\t" << (*property)[i] / valueUnit
                   << G4endl;
        }
    };
    ...
    if (lAr)
    {
        auto* larMPT = lAr->GetMaterialPropertiesTable();

        G4cout << "Configured base scintillation yield = "
               << fLArScintYieldPerMeV
               << " photons/MeV"
               << G4endl;

        G4cout << "Configured scintillation-yield scale = "
               << fLArScintYieldScale
               << G4endl;

        G4cout << "Configured effective scintillation yield = "
               << fLArScintYieldPerMeV * fLArScintYieldScale
               << " photons/MeV"
               << G4endl;
        ...
        G4cout << "Configured fast time constant = "
               << fLArFastTime / CLHEP::ns
               << " ns"
               << G4endl;

        G4cout << "Configured slow time constant = "
               << fLArSlowTime / CLHEP::ns
               << " ns"
               << G4endl;

        G4cout << "Configured fast fraction = "
               << fLArFastFraction
               << G4endl;

        G4cout << "Configured slow fraction = "
               << 1.0 - fLArFastFraction
               << G4endl;

        G4cout << "Stored Birks constant = "
               << lAr->GetIonisation()->GetBirksConstant()
                    / (CLHEP::mm / CLHEP::MeV)
               << " mm/MeV"
               << G4endl;

        G4cout << "Mean energy per ion pair = "
               << lAr->GetIonisation()
        ...
        if (larMPT)
        {
            G4cout << "\n[LAr constant properties stored in Geant4]"
                   << G4endl;

            if (larMPT->ConstPropertyExists(
                    "SCINTILLATIONYIELD")) {

                G4cout << "SCINTILLATIONYIELD = "
                       << larMPT->GetConstProperty(
                              "SCINTILLATIONYIELD")
                            * CLHEP::MeV
                       << " photons/MeV"
                       << G4endl;
            ...
            if (larMPT->ConstPropertyExists(
                ...
                G4cout << "SCINTILLATIONTIMECONSTANT1 = "
                       << larMPT->GetConstProperty(
                              "SCINTILLATIONTIMECONSTANT1")
                            / CLHEP::ns
                       << " ns"
            ...
            if (larMPT->ConstPropertyExists(
                ...
                G4cout << "SCINTILLATIONTIMECONSTANT2 = "
                       << larMPT->GetConstProperty(
                              "SCINTILLATIONTIMECONSTANT2")
                            / CLHEP::ns
                       << " ns"
            ...
            printPropertyVector(
                "LAr RINDEX",
                larMPT->GetProperty("RINDEX")
            );

            printPropertyVector(
                "LAr ABSLENGTH",
                larMPT->GetProperty("ABSLENGTH"),
                "cm",
                CLHEP::cm
            );

            printPropertyVector(
                "LAr SCINTILLATIONCOMPONENT1",
                larMPT->GetProperty(
                    "SCINTILLATIONCOMPONENT1")
            );

            printPropertyVector(
                "LAr SCINTILLATIONCOMPONENT2",
                larMPT->GetProperty(
                    "SCINTILLATIONCOMPONENT2")
            );
        }
        else {
            G4cout << "LAr material-properties table is null."
                   << G4endl;
        }
    }
    else {
        G4cout << "LAr material was not found."
               << G4endl;
    }
    ...
    G4cout << "Configured TPB efficiency = "
           << fTPBEfficiency
           << G4endl;
    ...
    {
        auto* tpbMPT =
            tpbMat->GetMaterialPropertiesTable();

        if (tpbMPT)
        {
            if (tpbMPT->ConstPropertyExists(
                    "WLSMEANNUMBERPHOTONS")) {

                G4cout << "WLSMEANNUMBERPHOTONS = "
                       << tpbMPT->GetConstProperty(
                              "WLSMEANNUMBERPHOTONS")
                       << G4endl;
            }
            ...
            printPropertyVector(
                "TPB RINDEX",
                tpbMPT->GetProperty("RINDEX")
            );

            printPropertyVector(
                "TPB WLSABSLENGTH",
                tpbMPT->GetProperty("WLSABSLENGTH"),
                "cm",
                CLHEP::cm
            );
            ...
            printPropertyVector(
                "TPB WLSCOMPONENT",
                tpbMPT->GetProperty("WLSCOMPONENT")
            );
        }
        else {
            G4cout << "TPB material-properties table is null."
                   << G4endl;
        }
    }
    else {
        G4cout << "TPB material was not found."
               << G4endl;
    }

    // --------------------------------------------------------
    // Reflectors and sensor configuration
    // --------------------------------------------------------
    G4cout << "\n------------- Reflectors and sensors -------------"
           << G4endl;

    G4cout << "ESR reflectivity = "
           << fReflectorReflectivity
           << G4endl;

    G4cout << "PMT reflectivity = "
           << fRefl_PMT
           << G4endl;

    G4cout << "SiPM reflectivity = "
           << fRefl_SiPM
           << G4endl;

    G4cout << "Configured top PMT PDE = "
           << fPDE_TopPMT
           << G4endl;

    G4cout << "Configured bottom PMT PDE = "
           << fPDE_BotPMT
           << G4endl;

    G4cout << "Configured SiPM PDE = "
           << fPDE_SiPM
           << G4endl;

    G4cout << "Active top PMTs = "
           << fTopPMTs
           << G4endl;

    G4cout << "Active bottom PMTs = "
           << fBotPMTs
           << G4endl;

    G4cout << "Active SiPM rows = "
           << fSiPMRows
           << G4endl;
