...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
 // G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
  ...
  G4double yPos = position.y()/CLHEP::cm;
  G4double zPos = position.z()/CLHEP::cm;
 // G4bool transmit = ((energy > 0.0 && fabs(yPos)<1.&& fabs(zPos)<1.) && (fDirX >= 0.0 && position.x() >= fXendAbs)); 
  
  G4bool transmit = (energy > 0.0 && fDirX >= 0.0 && position.x() >= fXendAbs);
  ...
  if (transmit){
      fEventAction->SetTransmitFlag(flag);
     // std::cout << "transmitted particle final pos is " << position/CLHEP::cm << " cm" << std::endl;
  }  
 ...
 // if (id>0) { analysisManager->FillH1(id, energy);}
  //if(id==10) {analysisManager->FillNtupleDColumn(0,energy);}
